...
## Slide 7 - Minggu Depan: Fase 1 Revisi Jadwal (presentasi 24 Juli)

**Usulan revisi jadwal (butuh persetujuan):** pipeline screening CV digeser ke
minggu 4 karena bergantung sampel CV historis tim DS (serah terima bertahap
baru mulai minggu 3). Alur maju lebih dulu memakai skor dummy - kontrak API
sudah final, skor nyata tinggal menggantikan.

Urutan baru: Minggu 2 = Alur + Gate + Email → Minggu 3 = Zoom + chatbot →
Minggu 4 = pipeline screening CV → Minggu 5 = konsolidasi (tetap).

Target 24 Juli: kandidat testing berjalan registrasi → keputusan Gate 1 (skor
dummy), email terkirim di tiap perubahan status.
